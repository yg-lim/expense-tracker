...
module.exports = class PgPersistence {
  ...
  async loadMonthsExpenses(year, month) {
    const GET_EXPENSES = "SELECT * FROM expenses" +
                         " WHERE date BETWEEN $1 AND $2 AND username = $3" + 
                         " ORDER BY date DESC";
    ...
    let result = await dbQuery(GET_EXPENSES, startOfMonth, endOfMonth, this.username);
    return result.rows;
  }

  async createNewExpense(description, amount, date) {
    const CREATE_EXPENSE = "INSERT INTO expenses (description, amount, date, username)" +
                           " VALUES ($1, $2, $3, $4)";

    let result = await dbQuery(CREATE_EXPENSE, description, amount, date, this.username);
  ...
  async loadExpense(expenseId) {
    ...
    let result = await dbQuery(FIND_EXPENSE, expenseId, this.username);
  ...
  async updateExpense(description, amount, date, expenseId) {
    ...
    let result = await dbQuery(UPDATE_EXPENSE, description, amount, date, expenseId, this.username);
    return result.rowCount === 1;
  }

  async deleteExpense(expenseId) {
    const DELETE_EXPENSE = "DELETE FROM expenses WHERE id = $1 AND username = $2";
    let result = await dbQuery(DELETE_EXPENSE, expenseId, this.username);
